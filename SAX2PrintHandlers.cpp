...
void SAX2PrintHandlers::error(const SAXParseException& e)
{
	...
	str.append(QString::number(line, 10));
	str.append(", char ");
    int colume;
	colume = e.getColumnNumber();
	str.append(QString::number(colume, 10));
		str.append("\n  Message: ");
    const XMLCh *strch;
	strch = e.getMessage();
	size_t len = wcslen(strch) + 1;
	size_t converted = 0;
	char *CStr;
	CStr = (char*)malloc(len*sizeof(char));
	wcstombs_s(&converted, CStr, len, strch, _TRUNCATE);
...
void SAX2PrintHandlers::fatalError(const SAXParseException& e)
{
	...
	XERCES_STD_QUALIFIER cerr << "\nFatal Error at file " << StrX(e.getSystemId())
		...
		<< ", char " << e.getColumnNumber()
		<< "\n  Message: " << StrX(e.getMessage()) << XERCES_STD_QUALIFIER endl;
	//errorList.append(QString(QString::fromLocal8Bit(cerr.c_str())));
	//str = QString::fromStdString(cerr);
}
...
void SAX2PrintHandlers::warning(const SAXParseException& e)
{
	QString str;
	//str.append("\nError at file ");
	//str.append((char*)StrX(e.getSystemId()));
	str.append("line ");
    int line;
	line = e.getLineNumber();
	str.append(QString::number(line, 10));
